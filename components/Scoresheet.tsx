...
const Scoresheet: React.FC<ScoresheetProps> = ({
  ...
  const overallGrandTotal = displayMultipliedGrandTotals.reduce((sum, total) => sum + total, 0);
  ...
  if (gameMode === 'classic' && effectiveNumColumns === 1) {
    ...
    const renderClassicCategoryRow = (category: Category, categoryIndex: number) => {
      ...
      return (
        <tr key={category.key} className={`hover:bg-slate-700/30 group ${category.key === ScoreCategoryKey.SIXES ? 'border-b-2 border-slate-500' : 'border-b border-slate-700/50'}`}>
          <td className="py-2 px-3 text-sm font-semibold transition-colors group-hover:text-white text-slate-300 min-w-[120px]">{category.label}</td><td className={scoreCellStyles} onClick={() => clickable && score === null && onSelectScore(category.key, 0)}><div className={scoreCellInnerDivStyles}>{scoreCellContent}</div></td>
        </tr>
      );
    };
    ...
    const renderClassicTotalRow = (label: string, value: number | string, highlight: boolean = false, isBonus: boolean = false) => (
        <tr className={`font-semibold ${highlight ? 'text-white bg-slate-500/80' : 'text-slate-100 bg-slate-600/80'}`}>
            <td className="py-1.5 px-2 text-base text-left align-middle border-b border-slate-500">{label}</td><td className={`py-1.5 px-2 text-base text-right align-middle border-b border-slate-500 ${isBonus && typeof value === 'number' && value > 0 ? 'text-green-400' : ''}`}>{isBonus && typeof value === 'number' && value > 0 ? `+${value}` : value}</td>
        </tr>
    );

    return (
      <div className="bg-slate-900/60 backdrop-blur-md rounded-2xl border border-slate-700/50 shadow-2xl overflow-hidden">
        ...
        return (
          <tr key={category.key} className={`${category.section === 'lower' && category.key === ScoreCategoryKey.THREE_OF_A_KIND ? 'border-t-2 border-slate-600' : ''}`}>
            <td className="py-1.5 px-3 border border-slate-600 bg-slate-800 font-semibold text-sm md:text-base sticky left-0 z-20 shadow-[2px_0_5px_rgba(0,0,0,0.3)] min-w-[110px] text-slate-100 text-left align-middle">{category.label}</td>{effectiveColumnMultipliers.map((_multiplier, colIdx) => { 
              ...
              return (
                <td key={colIdx} className={cellStyles} onClick={() => clickable && score === null && onSelectScore(category.key, colIdx)}><div className={innerDivClasses}>{cellContent}</div></td>
              );
            })}
          </tr>
        );
    ...
    return (
      <div className="overflow-x-auto bg-slate-900/60 backdrop-blur-md rounded-2xl border border-slate-700/50 shadow-2xl">
        <table className="min-w-full border-collapse">
          ...
          <tbody>
            ...
            <tr className="bg-slate-700 font-bold text-slate-100">
              <td className="py-2 px-3 border border-slate-600 sticky left-0 z-20 bg-slate-700 text-xs md:text-sm text-left align-middle shadow-[2px_0_5px_rgba(0,0,0,0.3)]">Summe Oben</td>{displayRawUpperSums.map((sum, idx) => <td key={idx} className="py-2 px-2 border border-slate-600 text-center text-sm align-middle">{sum}</td>)} 
            </tr>
            <tr className="bg-slate-700 font-bold">
              <td className="py-2 px-3 border border-slate-600 sticky left-0 z-20 bg-slate-700 text-xs md:text-sm text-slate-100 text-left align-middle shadow-[2px_0_5px_rgba(0,0,0,0.3)] border-t-2 border-slate-500">Bonus (&ge;{UPPER_SECTION_BONUS_THRESHOLD})</td>{displayRawBonuses.map((bonus, idx) => <td key={idx} className={`py-2 px-2 border border-slate-600 border-t-2 border-slate-500 text-center text-sm align-middle ${bonus > 0 ? 'text-green-400 font-black' : 'text-slate-400'}`}>{bonus > 0 ? `+${bonus}`: '0'}</td>)} 
            </tr>
            <tr className="bg-slate-800 text-emerald-400 font-bold">
              <td className="py-2 px-3 border border-slate-400 sticky left-0 z-20 bg-slate-800 text-xs md:text-sm text-left align-middle shadow-[2px_0_5px_rgba(0,0,0,0.3)]">Gesamt Oben</td>{displayRawUpperTotalsWithBonus.map((total, idx) => <td key={idx} className="py-2 px-2 border border-slate-400 text-center text-sm align-middle">{total}</td>)} 
            </tr>
            {CATEGORIES_CONFIG.filter(c => c.section === 'lower').map(renderCategoryRow)}
            <tr className="bg-slate-700 font-bold text-slate-100">
              <td className="py-2 px-3 border border-slate-600 sticky left-0 z-20 bg-slate-700 text-xs md:text-sm text-left align-middle shadow-[2px_0_5px_rgba(0,0,0,0.3)]">Summe Unten</td>{displayRawLowerSums.map((sum, idx) => <td key={idx} className="py-2 px-2 border border-slate-600 text-center text-sm align-middle">{sum}</td>)} 
            </tr>
            <tr className="bg-slate-800 text-yellow-400 font-bold">
              <td className="py-2 px-3 border border-slate-400 sticky left-0 z-20 bg-slate-800 text-xs md:text-sm text-left align-middle shadow-[2px_0_5px_rgba(0,0,0,0.3)]">Endsumme (x Multi)</td>{displayMultipliedGrandTotals.map((total, idx) => <td key={idx} className="py-2 px-2 border border-slate-400 text-center text-sm md:text-base align-middle">{total}</td>)} 
            </tr>
          </tbody>
        </table>
